...
class Instance:
    ...
    ``url_env`` / ``api_key_env`` / ``token_env`` are ENV VARIABLE NAMES, not
    values. ``optional_api_key`` marks an instance (e.g. jina) that may run
    without its key — it stays enabled even if the key var is unset.
    """
...
INSTANCES: list[Instance] = [
    ...
    Instance("serper", "serper", api_key_env="SERPER_API_KEY"),
    Instance("exa", "exa", api_key_env="EXA_API_KEY"),
    # --- read ---
    Instance("trafilatura", "trafilatura"),
    # jina works keyless (lower rate limit); the key is optional.
    Instance("jina", "jina", api_key_env="JINA_API_KEY", optional_api_key=True),
    Instance("crawl4ai", "crawl4ai", url_env="CRAWL4AI_URL", token_env="CRAWL4AI_TOKEN"),
    Instance("tavily-1", "tavily", api_key_env="TAVILY_1_API_KEY"),
    Instance("tavily-2", "tavily", api_key_env="TAVILY_2_API_KEY"),
    Instance("firecrawl", "firecrawl", api_key_env="FIRECRAWL_API_KEY"),
]
